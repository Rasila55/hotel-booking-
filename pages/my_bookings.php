...
<style>
    ...
    .booking-card:hover { transform: translateY(-5px); box-shadow: 0 0.5rem 1rem rgba(0,0,0,0.15) !important; }
    .status-badge { font-size: 13px; padding: 5px 12px; }
    .no-bookings { padding: 60px 20px; text-align: center; }
    .no-bookings i { font-size: 80px; color: #6c757d; margin-bottom: 20px; }
</style>
            ...
            <?php else: ?>
                <div class="row">
                    <?php while($booking = mysqli_fetch_assoc($result)):
                        ...
                        if ($booking['status'] == 'completed') $badge_class = 'bg-info text-dark';
                        ...
                        <div class="card booking-card shadow-sm border-0 h-100">
                            <div class="card-body">
                                ...
                                <div class="mb-2">
                                    ...
                                    <small><strong>Check-in:</strong> <?php echo date('d M Y', strtotime($booking['check_in'])); ?></small>
                                </div>
                                <div class="mb-2">
                                    <i class="bi bi-calendar-event me-2 text-danger"></i>
                                    <small><strong>Check-out:</strong> <?php echo date('d M Y', strtotime($booking['check_out'])); ?></small>
                                </div>
                                <div class="mb-2">
                                ...
                                <!-- ══════════════════════════════════════
                                     ...
                                     pending   + upcoming = show Cancel button
                                     confirmed + upcoming = show locked badge (admin confirmed, cannot cancel)
                                     cancelled            = show Cancelled badge
                                     completed or past    = show Completed badge
                                ══════════════════════════════════════ -->
                                <div class="mt-3">
                                    <?php if ($booking['status'] == 'pending' && $is_upcoming): ?>
                                        <!-- PENDING: can cancel -->
                                    ...
                                    <?php elseif ($booking['status'] == 'confirmed' && $is_upcoming): ?>
                                        <!-- CONFIRMED by admin: cannot cancel -->
                                        <div class="alert alert-success py-2 mb-0 text-center" style="font-size:12px;">
                                            <i class="bi bi-shield-check me-1"></i>
                                            <strong>Confirmed by Admin</strong><br>
                                            <span class="text-muted" style="font-size:11px;">This booking cannot be cancelled</span>
                                        </div>

                                    <?php elseif ($booking['status'] == 'cancelled'): ?>
                                        <!-- CANCELLED -->
                                        <span class="badge bg-danger w-100 p-2" style="font-size:12px;">
                                            <i class="bi bi-x-circle me-1"></i>Booking Cancelled
                                        </span>

                                    <?php elseif (!$is_upcoming || $booking['status'] == 'completed'): ?>
                                        <!-- PAST / COMPLETED -->
                                        <span class="badge bg-secondary w-100 p-2" style="font-size:12px;">
